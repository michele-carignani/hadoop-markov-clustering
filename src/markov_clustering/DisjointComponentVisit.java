...
public class DisjointComponentVisit {
	...
	public enum Style {
		VERTICAL, HORIZONTAL, CIRCLES, CROSS, FILLED, LEFT_DIAG, RIGHT_DIAG
	}
	...
	private void drawPatchwork(Clustering c) throws IOException {
		...
		c.cpw.setBackground(Color.black);
	...
	*/
	...
	public void generateColors() {
		...
		for(Color c : this.colors){
			...
			this.fillers.add(new Filler(c, Style.CIRCLES));
	...
	public void assign_random_colors(Clustering first) {
		...
		for(Cluster c : first.clusters){
			c.filler = this.fillers.get(r.nextInt(this.fillers.size()));
		}
	}
